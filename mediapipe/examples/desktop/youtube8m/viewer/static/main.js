 ...
 * @param {number} index 0-based index in a tfrecord.
 */
function fetchVideo(filePath, index) {
  const url = "/video?file=" + filePath + "&index=" + index;
  console.log('fetching', url);
  $.ajax({
    url: url,
    success: function(result) {
      const videoId = result["video_id"];
      console.log('video id is now:', videoId);
